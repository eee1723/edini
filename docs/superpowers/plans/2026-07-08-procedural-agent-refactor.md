...
| 1a 参数引用 | ✅ 完成 (2026-07-08) | 原型→相对 ch()(可迁移);手建保持绝对+repath |
| 3 emit_markers | ✅ 完成 (2026-07-08) | builder.emit_markers + add_anchors 跨调用覆盖 bug 修复;3 hython 铁证 |
| 4 组件原型 | ✅ 完成 (2026-07-08) | box_panel/copy_array/tube_graph 三原型;extrude_profile 延后 |
| 2 verify_robust | ✅ 完成 (2026-07-08) | node_utils.verify_robust + harness.ts;3 hython 铁证 |
| 5 组件迭代 | ✅ 完成 (2026-07-08) | snapshot/restore/list 平台工具;UI 版本列表留作增强 |
...
### ✅ Phase 4 — 组件原型化(2026-07-08)

**目标**:把 step 3(组件内部建模,占 agent 80% 工作)从"裸 create_node/set_param + prompt 模板"升级为声明式原型生成器——治 step 3 头号失败(Python SOP 错误 7+ 次 / 错参名 / 断 ch 链)。这是"发挥 agent 最大能力"的核心杠杆。

**实现**:`builder.emit_component(core, component_id, archetype, params)` dispatch + 3 原型(每个确定性节点名,replace-on-rebuild 幂等;值约定:数字=字面量,字符串=design_param 名→绝对 ch() 引用):
- **box_panel**:参数化 box(桌面/座面/挡板)。`size=[x,y,z]` 各为数字或参数名;可选 `markers` 转发 emit_markers。
- **copy_array**:CTP 把 leaf 盖到组件消费的锚点(桌腿/辐条/键)。`leaf={type,params}`;收集 `in_<from>_<anchor>` null → merge → CTP(读 @orient/@scale/@N)。
- **tube_graph**:消费命名锚点间建管子图(车架/fork/把手)。`tubes=[{a,b}]` + `radius`。**用 VEX detail wrangle 替代 Python SOP**(`__findpt` 按 @name 找点 + addprim polyline)→ polywire 加粗——零 Python SOP 错误面(比原计划的 Python SOP 骨架更稳)。

**延后**:`extrude_profile`——与 tube_graph(polywire 管子)重叠;真正有区分度的"自定义截面 sweep"较 niche 且 sweep::2.0 有 H21 surfaceshape 坑。dispatch 框架支持未来干净追加。

**验证**:`795 passed`。**9 条真机 hython 铁证**:box_panel 参数化(sizex=length→1.2)+ LIVE(改 length→bbox 2.0)+ 幂等 + marker 转发;copy_array 4 锚点→4 实例(24 prims);tube_graph 3 边管子图无错构建 + radius 参数驱动 bbox 增长。`tool_executor` 注册 + `project.ts` TS 工具(3 原型 + promptGuidelines)。

### ✅ Phase 1a — 参数引用方案(原型相对引用,稳进)(2026-07-08)

**目标**:让组件"可迁移"——从绝对 `ch('/obj/.../project_core/<p>')`(绑死 scene 路径,改名/复制即断)改为相对 `ch("../../<p>")`(按位置引用,复制到别的 project 仍 cook)。

**稳进做法**(低风险):
- **原型生成器**(`_set_archetype_parm`)的字符串值(参数名)分支:从绝对 `ch("{core_path}/{p}")` 改为**相对** `ch("{rel}{p}")`,其中 `rel` 由 `_relative_path_to_core(node, core)` 按节点实际深度计算(原型节点是组件子网的直接子节点 → depth-2 `../../`)。原型节点深度由原型契约保证(都是直接子节点),稳。
- **手建路径**(agent raw 节点)**保持绝对**——不碰有风险的手建深度计算;agent 优先用原型(Phase 4,自动相对),无原型匹配时手建 + 按需 `project_repath_to_relative` 迁移。

**验证**:`798 passed`。现有原型测试全绿(相对 ref 解析等价:sizex 仍 == 1.2、LIVE 跟随)。**3 条可迁移性真机 hython 铁证**:原型生成的表达式是相对 `../../length`(非绝对 `/proj_mA/`);proj_A(length=1.2)的 box_panel 复制到 proj_B(length=3.0)→ 副本读 proj_B 的 length(**3.0 非 1.2**,绝对引用必断或读旧值);迁移后改 proj_B length→2.5 副本跟变(LIVE 在新家)。

### ✅ Phase 5 — 组件级迭代(snapshot/restore)(2026-07-08)

**目标**:"有选择的多轮优化"——快照一个组件的当前状态,迭代,若新版更差则 restore,无需重跑整个 project。

**实现**:3 个平台工具(`builder.snapshot_component` / `restore_component` / `list_snapshots`)+ tool_executor 注册 + project.ts TS 工具:
- 快照存于 core 内的 `_snapshots` 子网(非声明组件 → OUT/inspect 跳过,随 .hip 持久化)。
- `copyNodesTo` 保留内部接线 + 外部 ports.in 连接(按路径);相对 ch() 在恢复后深度自洽(depth-2)。
- `snapshot_id = "<component>_<N>"`;可选 label 存节点 comment;list 解析回 [{id, component, label}]。

**验证**:`801 passed`。**3 条真机 hython 铁证**:快照(sizez=0.6)→ 改(sizez=2.0)→ restore 回退到 0.6(快照胜);恢复后 sizex 仍读 length=1.2(相对 ref 幸存);list_snapshots 记录 top_1。踩坑:copyNodesTo/destroy 会使持有的 Python 节点 handle 失效(hou.ObjectWasDeleted)→ 平台工具无状态(每次调用自 lookup,agent 不受影响),测试 harness 改为跨变更操作重 lookup。

**留作增强**:UI 版本列表(NodeVersionList 下沉到组件级,可视化 snapshot 管理)——平台能力已就绪,UI 是上层展示。
