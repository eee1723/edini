...
**关键技术点：subnet 的输出端不是凭空存在的，必须由 subnet 内部的 `output` 节点定义。** subnet 对外的每个 output connector，对应内部一个 `output` 节点的输出连线。所以"建 2 个输出端" = "建 2 个 `output` 节点，各自接上游几何"。

builder 对每个 component 建如下脚手架（subnet 内部，共 4 个节点）：

```
<component>/  (subnet)
├─ out_geometry   (null 节点)            ← builder 建，空，等 LLM 在上游填主几何
├─ out_anchors    (null 节点)            ← builder 建，空，等 LLM 在上游填锚点云
├─ output_0       (output 节点) ←─ 接 out_geometry → 形成 subnet output connector 1
└─ output_1       (output 节点) ←─ 接 out_anchors  → 形成 subnet output connector 2
```

连线（builder 建）：`out_geometry → output_0`、`out_anchors → output_1`。

- builder **不建**任何 VEX/box/torus（旧 build_assembly 的活，现在归 LLM）。
- builder 建这 4 个节点 + 2 条连线，使端口协议物理成立（subnet 真正有 2 个输出端）。
...
promote 是**独立触发的脚本**（不是每次 build 自动跑）。默认时机：**LLM 完成一轮建模任务后自动跑一次**（把该轮新暴露在组件 subnet 上的 spare parm promote 到 core HDA）；用户也可随时手动触发（想刷新变体接口时）。
...
1. builder 收到声明（2 components + ports + params）
   → core 内建 2 个 subnet（chassis/, wheels/），各带 4 节点脚手架：
     out_geometry/out_anchors (null) + output_0/output_1 (output 节点) + 2 条连线
   → 断言：./chassis、./wheels 存在；每个 subnet 内有 2 null + 2 output 节点；
     output_0 接 out_geometry、output_1 接 out_anchors；subnet 真有 2 个输出端
...
| 风险 | 对策 |
|---|---|
| subnet 多 output 节点建网/连线的真实 API 细节未验证（建多个 `output` 节点 + setInput） | §8 步骤 1 的 hython 验证先行暴露；参考 Houdini 自带 subnet 内部多 output 用法（subnet 容器内 `output` 子节点类型） |
