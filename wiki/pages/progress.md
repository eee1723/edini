...
> 最后更新：2026-07-09&nbsp;|&nbsp; **🟢 Houdini 卡死根治 — 工具执行异步化 + 流式/思考渲染三重 bound（5 轮迭代收敛）**。症状：agent 运行时 Houdini 整个冻结、越用越卡。会话日志时间线（把每段间隔按"工具执行 vs agent 响应"分类）+ 第一性原理定位：Edini 面板**共享 Houdini 主线程**，任何主线程重活都冻整个 Houdini。三层饥饿逐一 bound：① **工具执行放大**——旧同步 `do_POST` + 客户端 30s 超时重试 → 重工具（`project_finalize` ≈21 recook）孤儿化重跑最多 3 次、锁死数分钟、后续全 `fetch failed`。修复：**异步 job/poll 协议**（`tool_executor.py` 新增 `ToolJobExecutor`+drain 线程：`POST /execute` 立即回 job_id、`GET /result/<id>` 轮询；`_shared.ts:forwardTool` 拿到 job_id 后**永不重发重活**，放大结构上不可能）。② **工具编排到主线程（中途引入的回归，已回退）**——一度用 `hou.executeInMainThreadWithResult` 投递主线程，排在流式渲染 timer 后面 → `set_param` 等到 40s、越用越卡；回退为**单一后台 drain 线程串行执行**（hou 安全、永不碰主线程）。③ **流式/思考渲染**——`QTextEdit.setHtml` 全文档重排 O(n)，glm high-thinking 每轮上万 token →"思考更新时冻 Houdini"（用户实测点破）；**三重 bound**：频率（思考 120ms 节流，文本已有 80ms）/ 跨轮（`begin_assistant_message` 每回合清空 `_thinking_full`，治"越用越卡"）/ 单次成本（`ThinkingPanel` 渲染截断到尾部 2000 字符，完整思考在时间线）。**验证**：**848 passed（+11 新测：异步协议 6 + 渲染节流/截断 5），零回归**，含决定性 hython。诚实提示：剩余"慢"是 glm-5.2 high-thinking 每轮上万 token 的 LLM 生成时间（单轮 200s+），Edini 改不了——降 thinking level / 换快模型是唯一杠杆；线程修复只让 UI 在等待期间保持响应。详见 [踩坑记录](pitfalls.md) Houdini 主线程段。
>
> 上一轮（2026-07-09）&nbsp;|&nbsp; **🟢 第一性原理开发计划 — Phase 5 完成：闭环学习（5a 失败→知识 + 5b 成功→原型）**。两个独立半环,均**纯增量**(不改任何现有返回形状)。**5a 失败→知识草稿**:之前 agent 根本无法写知识库(只有 `edini_search_knowledge` 读),知识库无 draft 状态(rule/entry 是类型非生命周期),失败是无结构字符串。落地:① `knowledge_store.py` 加 **draft 层**(entries 增 `status` 字段,缺省 promoted 向后兼容;`search_entries` 默认**排除** draft 避免污染 agent 常规搜索,`drafts_only=True` 专查;`add_failure_draft(s)` 把结构化失败记录写成 `status=draft` 的"避坑"条目 + `failure` 结构 blob;`promote_entry(to_rule=)` 提升);② `project_finalize` **自动草稿闸**(用户选)——每条 gate 失败经 `_add_failure(category,...)` 记结构化 FailureRecord(category 词表:incomplete/cook_error/degenerate/dead_param/orientation/missing_param + 修复 hint),收尾 `add_failure_drafts(records, project_context)`(包 try/except,知识写失败绝不阻断 finalize;返回新增 `drafts_created`/`failure_records`,加性不改原 shape);③ 新工具 `edini_search_drafts`(tool_executor + knowledge.ts + index.ts 注册)——agent 卡住时查历史失败诊断;④ `KnowledgeZone` 加 **Drafts 折叠区** + 每条"提升/铁律"按钮(人工 promote)。**5b 成功→原型捕获**:`project_capture_archetype(core, component_id, name)` —— 遍历组件子网,用 ports.py 常量**滤掉**脚手架/输入/锚点/marker 管线节点,每个原型节点发一个 `node` op + 终末 `wire_out`(从 out_geometry 沿 input-0 追过 markers_merge 找真源)。**Tier-2 参数引用恢复**:parm 的 ch() 表达式若引用已声明 design_param,恢复成裸 design_param 名(重发时 `_set_archetype_parm`→相对 ch(),深度自洽可迁移);字面量照烤。踩坑:真实 hou `Parm` **无 `hasExpression()`**(recipe_library 的 `hasattr` 防御在真机恒 False),改用 `expression()` try/except。**sidecar 注册表**:捕获 spec 存 `~/.pi/agent/edini-archetypes/<name>.json`(数据,镜像 recipe 库模式),`load_spec` sidecar 优先 + 包模块回退,`list_archetypes` 并集——捕获的原型**立即可被 `emit_component` 复用**。`state.py` 加 `archetype_candidates` 引用 + 审计 log。新工具 `project_capture_archetype`/`project_list_captured_archetypes`(tool_executor + project.ts)。**范围诚实**:高层 op 识别(`vex_tube_graph`/`collect_anchors`)**故意不做**(脆弱低价值),捕获的 tube_graph 降级为字面 node op(仍正确重发)。**验证**:**836 passed + 2 subtests,零回归**(825 + 11 新测:6 knowledge draft + 2 finalize→draft hython + 3 捕获 round-trip hython)。**5 条真机 hython 铁证**:finalize 失败自动写 draft(incomplete 类别 + 含 legs + tag auto-draft)/ draft 被 `edini_search_knowledge` 隔离但 `edini_search_drafts` 查到;捕获 box_panel → spec 只剩 panel_box + wire_out + sizex/sizey/sizez 恢复成裸 length/thickness/width / load_spec+list 立即见 captured_box / 重发 captured_box 复现等价几何 + length→3.0 两 box 都跟(LIVE 参数化幸存)。edini-context 加闭环学习引导(失败查 drafts / 成功捕获原型)。第一性原理开发计划 **Phase 1-5 全部完成**。
>
> 上一轮（2026-07-08 晚）&nbsp;|&nbsp; **🟢 第一性原理开发计划 — Phase 4 完成：node_utils.py 拆分（机械重构零行为变更）**。把 4627 行的 `node_utils.py` 按职责拆成 4 模块 + re-export shim,治"单文件 4600 行不可维护"。**拆分**:`node_ops.py`(1943,节点 CRUD/场景/脚本执行/截图)/`manifest.py`(1035,parm 目录生成·加载·查询+H21 版本解析)/`geometry_inspect.py`(489,几何读取/health)/`verify.py`(1157,验证+项目状态/闸门);`node_utils.py` 降为 **38 行 re-export shim**。**纯机械重构**:AST 脚本 `scripts/_phase4_split_node_utils.py` 一次拆完,**85 个顶层函数全对账**(无丢失/无重复/函数体行数全等),零行为变更。**跨模块依赖(执行前 grep 验证,确认无环)**:geometry_inspect/manifest 是 leaf;verify→geometry_inspect(`_vector_to_list`);node_ops→manifest(`_json_safe`/`manifest_parm_names`/`_node_parm_inventory`)+geometry_inspect(`geometry_inventory`,capture_component_detail 用)。**shim 设计关键**:`from .X import *`(公开面)+ **动态镜像下划线私有名**——因测试与 builder/archetype_emitter 按名 import 大量私有 helper(`_apply_one_param`/`_relative_path_to_core`/`_CH_CALL_RE`/`_frame_to_bounds`/`_HEALTH_BLOCKING_CHECKS`/`_make_replacer` 等),`import *` 默认跳过下划线,动态 `dir()` 镜像保证全留存。**两处测试调整(私有 helper 的 monkeypatch 跟随模块身份)**:① `test_node_utils` 两个 manifest 查询类把 `_node_parms_manifest_path` 的 patch 从 `edini.node_utils` 改到 `edini.manifest`;② `conftest.reload_edini_modules("edini.node_utils")` 现在连带驱逐 4 个**兄弟**实现模块。**PCA 保留不删**(live crosscheck,非死代码)。**验证**:**825 passed + 2 subtests,零回归**(含 79 hython)。
>
> 上一轮（2026-07-08 晚）&nbsp;|&nbsp; **🟢 第一性原理开发计划 — Phase 3 完成：project_plan 意图闸**。治 P2(上游错误复合放大)。新增 `project_plan(core_path, goal, success_criteria)` —— scaffold 前强制捕获建模意图,平台校验 goal+success_criteria 非空(空则拒,无 building 直到达成标准被声明),存 `declaration.success_criteria`(可被 project_finalize 后续 cross-check)。state.py 加 success_criteria 字段。node_utils 实现 + tool_executor 注册 + project.ts TS 工具 + edini-context 工作流链(create→**plan**→scaffold→...→**finalize**)。3 hython 铁证(捕获存储 / 拒空 criteria / 拒空 goal)。**新基线 825 passed(79 hython + 746 mock/logic),零回归**(822 + 3 project_plan)。剩 Phase 4(拆 node_utils,可延后)/5(闭环,DP2)。
>
> 上一轮（2026-07-08 晚）&nbsp;|&nbsp; **🟢 第一性原理开发计划 — Phase 2(2b/2c/2d)完成：P0 archetype 全部数据化**。承接 DP1(spec 格式批准 + param_specs 校验增强)。**2b**：迁移 copy_array + tube_graph 到 spec —— 补 `collect_anchors`/`vex_tube_graph` op + 扩展 `node` op(type/$ref-params/inputs/tweaks/init_ctp_attribs)，移除 `_archetype_copy_array`/`_archetype_tube_graph`/`_build_tube_graph_vex` legacy 代码，`emit_component` 全 spec 驱动(无 legacy 回退)。5 hython 测零回归(vector leaf live 表达式 / 未知 parm loud-fail / tube_graph edges + radius 驱动)。**2c**：新增 `extrude_profile`(柱/管，rad1+height+tube→Polygon tweak)——**仅加 `archetypes/extrude_profile.py` spec 文件，零 emitter 改动**即工作，验证 P0 可扩展性赌注(加原型=加数据)。2 hython 测(参数化 + LIVE)。**2d**：ch() 统一由 emitter 完成(spec 全相对 ch()；box_panel 的 absolute-ch() stale docstring 随函数删除消除)；手搓保持绝对+repath(按设计)；更新 project.ts/builder.py 文档(4 原型全落地)。**Phase 2 完成 —— `builder.py` 不再有硬编码原型分发，加原型=加 spec 文件**。**新基线 822 passed = 76 hython-decisive + 746 mock/logic，零回归**(819 + 1 bad-size + 2 extrude_profile)。剩 Phase 3(意图闸)/4(拆 node_utils)/5(闭环,DP2)。
>
