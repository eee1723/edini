...
**当前阶段**：105+ 单元测试 · edini_search_knowledge · 评估→知识联动
**下一阶段**：Houdini Pane Tab 嵌入 · Judge 模型优化
**工作分支**：`master`

## 一句话总结

Edini 是 Houdini 21 的 AI 助手：Python/PySide6 面板 → JSON-RPC 进程通信 → Pi Agent AI 后端 → 22 个 Houdini 工具。知识沉淀采用两层架构（铁律 + 知识库），AI 反思后用户面板确认。
...
- ✅ **测试**：Mock Hou 模块 + 105+ 单元测试（node_utils 48 + config 24 + knowledge_store 33）
