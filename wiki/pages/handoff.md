...
**最后更新**：2026-07-03（**工作区感知 + 参数自底向上 + HDA按钮弹窗** — 三个 UX 根基重做。project_create 复用选中 HDA(不总新建);参数自底向上(subnet 建→promote 按分组提到 core 带 min/max);HDA 参数面板加 💬 Chat 按钮 → 精简对话弹窗(取代原生 pane 主入口)。真机铁证:promote 分组+min/max+live；button+PythonModule 注入成功）。
