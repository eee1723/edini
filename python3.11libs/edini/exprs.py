"""Safe arithmetic expression engine for asset parameter linkage.

Asset authors reference asset-level parameters inside skeleton-point
position expressions, e.g.::

    "rear_axle":  {"expr": ["base[0]", "wheel_radius", "0"]}
    "front_axle": {"expr": ["rear_axle[0] + wheelbase", "wheel_radius", "0"]}
...
Design constraints:
  ...
  - The whitelist is deliberately small. If an asset needs more (e.g.
...
__all__ = ["ExprError", "evaluate", "evaluate_tuple", "extract_refs"]


class ExprError(ValueError):
    """Raised when an expression is unsafe or references an unknown name."""
...
def _eval_node(node: ast.AST, params: Mapping[str, float]) -> float:
    ...
    # Subscript — ONLY ``name[<int>]`` is allowed. This is the point-axis
    # reference syntax used by skeleton points (``rear_axle[0]``). The base
    # must be a Name (a resolved point bound to a tuple), and the index must
    # be a plain integer literal. String keys, slices, attribute subscripts,
    # and chained subscripts are all rejected — this keeps the security
    # boundary intact while permitting the one legitimate subscript form.
    if isinstance(node, ast.Subscript):
        if not isinstance(node.value, ast.Name):
            raise ExprError(
                "only a simple name may be subscripted (point reference); "
                f"got {type(node.value).__name__}")
        base_name = node.value.id
        if base_name not in params:
            raise ExprError(
                f"unknown name {base_name!r} in subscript (not a resolved "
                f"point or parameter)")
        base_val = params[base_name]
        # The index: accept an integer constant only.
        idx_node = node.slice
        # py>=3.9 wraps the index in ast.Subscript.slice directly (no Index).
        inner = idx_node.value if isinstance(idx_node, ast.Index) else idx_node
        if not isinstance(inner, ast.Constant) or not isinstance(inner.value, int):
            raise ExprError(
                "point reference index must be an integer literal "
                f"(e.g. name[0]), got {ast.dump(inner)}")
        idx = inner.value
        try:
            return float(base_val[idx])
        except (IndexError, TypeError) as exc:
            raise ExprError(
                f"cannot index {base_name!r}[{idx}]: {exc}") from None

    # Anything else (Attribute, Import, BoolOp, Compare, lambda,
    # comprehensions, ...) is rejected — this is the security boundary.
    # (Subscript on a plain name by an int literal IS allowed above — it is
    # the point-axis reference syntax ``rear_axle[0]``; every other subscript
    # form is rejected.)
    raise ExprError(
...
def evaluate_tuple(
    ...
    *,
    ...
    or a list of plain numbers (passed through). This lets skeleton-point
    fields accept both ``"expr": ["wheelbase/2", "wheel_r", "0"]`` and
    legacy numeric lists where applicable.

    Args:
        exprs: list of strings/numbers, length must equal ``length``.
        params: declared asset parameters (primary + resolved derived).
        length: expected tuple length (e.g. 3 for position).
...
# ── Reference extraction for dependency graph ──────────────────────
...
_BUILTIN_NAMES: set[str] = {
    ...
    # extended math funcs in _SAFE_FUNCS
    "asin", "acos", "atan", "atan2", "floor", "ceil", "exp", "log", "hypot",
    "round",
}
